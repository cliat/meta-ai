(function () {
  globalThis.metaAiCustomAnimate = async function (input) {
    ...
    if (!customAnimateButton) {
      return {
        ok: false,
        reason: "Custom animate button was not found.",
        buttons: listButtons(),
      };
    }
  ...
  function setTextboxValue(textbox, value) {
    ...
    if ("value" in textbox) {
      ...
      dispatchTextInputEvents(textbox, value);
      return;
    }
    ...
    if (!inserted || !textbox.innerText.includes(value)) {
      textbox.textContent = value;
      dispatchTextInputEvents(textbox, value);
    }
  }

  function dispatchTextInputEvents(textbox, value) {
